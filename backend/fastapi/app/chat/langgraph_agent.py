...
class LangGraphReActAgent:
    ...
    def get_graph_structure(self) -> Dict[str, Any]:
        ...
        for edge in graph.edges:
            ...
            # Determine connection handles based on ReAct pattern
            source_handle = None
            target_handle = None
            ...
            # Start -> Agent: Start right -> Agent left
            if source == START_NODE:
                source_handle = "right-source"
                target_handle = "left-target"
            # Agent -> Tool: Agent bottom -> Tool top (ReAct: Agent calls tool)
            elif source_lower == "agent" and (target_lower == "tools" or target_lower == "tool"):
                source_handle = "bottom-source"
                target_handle = "top-target"
            # Tool -> Agent: Tool top -> Agent bottom (ReAct: Tool returns result)
            elif (source_lower == "tools" or source_lower == "tool") and target_lower == "agent":
                source_handle = "top-source"
                target_handle = "bottom-target"
            # Agent -> End: Agent right -> End left
            elif source_lower == "agent" and target == END_NODE:
                source_handle = "right-source"
                target_handle = "left-target"
    ...
    async def astream_with_history(
        ...
        # Track the full state to access all messages when needed
        ...
        async for event in self.graph.astream_events(
            ...
            # Update current state from event if available
            if "data" in event and "output" in event["data"]:
                output = event["data"]["output"]
                if isinstance(output, dict) and "messages" in output:
                    # This is the node's return value, which gets merged into state
                    if current_state is None:
                        current_state = {"messages": messages}
                    # Merge the new messages into state
                    current_state["messages"] = list(current_state.get("messages", [])) + output["messages"]
            kind = event["event"]
            ...
            if kind == "on_chain_start":
                ...
                # Always check for final response when agent node ends
                if node_name == "agent":
                    # Try to get output from event data
                    # The output from _agent_node is {"messages": [response]}
