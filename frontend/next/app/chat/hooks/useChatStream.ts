...
export function useChatStream(options: ChatStreamOptions): ChatStreamReturn {
  ...
  const sendMessage = async (content: string, files?: ParsedFile[]) => {
    ...
    // Clear suggestions from previous assistant messages when starting a new message
    setMessages((prev) => 
      prev.map((msg) => 
    ...
    try {
      ...
      const finalizeMessage = (content: string) => {
        if (content && content !== thinkingText) {
          ...
          // Only update if content is different from current message content
          // This prevents overwriting with duplicate content
          setMessages((prev) =>
            prev.map((msg) => {
              if (msg.id === assistantMessageId) {
                // If current content is already set and matches, don't update
      ...
      while (!streamEnded) {
        ...
        if (done) {
          // Process remaining buffer - parse as SSE message format
          if (buffer.trim()) {
            ...
            // Only process token events from buffer
        ...
        for (const message of parts) {
          ...
          if (eventType === 'start') {
            ...
            // Token event - stream content chunk
            try {
              const tokenData = JSON.parse(data);
              if (typeof tokenData === 'string') {
                if (isFirstToken) {
                  isFirstToken = false;
                  accumulatedContent = tokenData; // Replace thinking text with first token
                } else {
                  accumulatedContent += tokenData;
                }
                chunkCount++;
          ...
          // Handle other data events (only if eventType is not token and not empty)
          // Note: token events are handled above, so this should not process token events
          // Also skip if eventType is empty (likely a continuation or malformed event)
          if (data && eventType !== 'token' && eventType !== '') {
            let processedData = '';
            try {
